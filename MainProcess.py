...
for filename in os.listdir(input_dir):
    ...
    if file_path.lower().endswith(supported_formats):
        ...
        if image is not None:
            print(f"成功讀取影像: {filename}")
        else:
            print(f"無法讀取影像: {filename}")
    else:
        print(f"不支援得圖片格式: {filename}")
# 將圖像放大1.5倍
...
# 轉換灰階
    ...
    # 以圖片名稱建立子資料夾
    child_slice_dir = os.path.join(slice_dir, filename)
    os.makedirs(child_slice_dir)
    # 保存非離群值影像
    for x, y, w, h in images_info:
        ...
        output_path = os.path.join(child_slice_dir, f'{filename}_cropped_{counter_str}_x{x}_y{y}_w{w}_h{h}.jpg')
    ...
    # 影像座標文檔(開發用)
    info_file = os.path.join(child_slice_dir, f'{filename}images_info.txt')
    ...
    txts = recognize_text(child_slice_dir)


    data_to_excel(child_slice_dir,images_info, txts)
# cv2.waitKey(0)